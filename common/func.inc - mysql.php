<?php
	class jabarsoft{
		...
		function pembulatan($uang){
			if($uang!=0){
				$pbawah=500;$patas=1000;
				$ratusan = substr($uang, -3);
				if($ratusan<=$pbawah){
					$akhir = $uang+($pbawah-$ratusan);
				}else{
					$akhir = $uang+($patas-$ratusan);
				}
			}else{
				$akhir=0;
			}
		...
		function Login()
		{
			...
			{				
				...
				{						
					...
					$UserNm=$r['nama_lengkap'];$UserHak =$r['idgroupakses'];							
				}			
				...
				{	
					...
					$QCekLog=_mysql_query("select * from __log_server where username='{$uLogin}' and date(waktu)=curdate()");
					if(!_mysql_num_rows($QCekLog)){
						_mysql_query("INSERT INTO __log_server (ipaddress, username, link_url, link_sub, waktu) VALUES ('".$_SERVER["REMOTE_ADDR"]."', '{$uLogin}', (SELECT KdUnit FROM __t_users WHERE username='{$uLogin}'), 'beranda', now());");
					}else{
						_mysql_query("UPDATE __log_server SET waktu=now() where username='{$uLogin}' and date(waktu)=curdate()");
					}
