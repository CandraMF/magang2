<?php
	...
	$wh=!empty($ckata)?" and nmgroupakses like '%$ckata%'":""; 
	$Qry="SELECT * from __t_group_akses where idgroupakses!='1' $wh ORDER BY nmgroupakses "; 
	...
	while($Isi=_mysql_fetch_array($Qry)){ 
		...
		$ListMode.=" 
 		...
 		"; 
		if($Excel!='yes'){ 	
			$onclick_edit="onclick=\"javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Lihat/{$Isi['idgroupakses']}')\"";
			$onclick_hapus="onclick=\"Cek=confirm('Apakah yakin akan menghapus data?');if(!Cek){return false;}else{javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Hapus/{$Isi['idgroupakses']}')}\"";
			$ListMode.=" 
				<td>
