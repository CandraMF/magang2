<?php
	...
	$stskamus=!empty($stskamus)?$stskamus:"Pengajuan";
	$aktif=!empty($aktif)?$aktif:"Aktif";
	$Readonly="";
	switch($Aksi)
	{
		case "Simpan";
			if(!empty($nmkamus)&&!empty($uraian)&&!empty($idbidang)){
				...
				if ($jCount > 0)
				{$Qry="UPDATE t_kamus_hukum SET nmkamus='{$nmkamus}', uraian='{$uraian}', stskamus='{$stskamus}', aktif='{$aktif}', updated_by='{$pupdated_by}', updated_date='{$pupdated_date}' WHERE  kdregister='{$idPop}';";}
				else
				{
                                ...
                                <div class='card-body'>
                                    ...
                                    {$Pesan}
										...
										<form name=Fm id=Fm method=post action='{$Url->BaseMain}/{$Pg}/{$Pr}/{$Mode}/{$Sb}' enctype='multipart/form-data'> 
											...
											<div class='row'>
												<div class='mb-3 mb-lg-3'>
													<label class='form-label' for='project-title-input' style='color:red;'>Status</label>
													".$Func->cmbUmum('stskamus',@$stskamus,array('Pengajuan','Ditolak','Diterima'))."
												</div>
											</div>
											<div class='row'>
												<div class='mb-3 mb-lg-3'>
													<label class='form-label' for='project-title-input' style='color:red;'>Aktif</label>
													".$Func->cmbUmum('aktif',@$aktif,array('Aktif','Tidak Aktif'))."
												</div>
											</div>
											<div class='row'>
