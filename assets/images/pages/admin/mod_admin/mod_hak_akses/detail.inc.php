<?php
	...
	{
		case "Simpan";
			if(!empty($nmgroupakses)){
				$QTmp="select count(*) as jml from __t_group_akses where idgroupakses='{$idPop}'";
				$Qry = _mysql_query( $QTmp );	
				$Hasil=_mysql_fetch_array($Qry);	$jCount=!empty($Hasil['jml'])?$Hasil['jml']:0;
			
				if ($jCount > 0)
				{$Qry="UPDATE __t_group_akses SET nmgroupakses='{$nmgroupakses}', updated_by='{$pupdated_by}', updated_date='{$pupdated_date}' WHERE  idgroupakses='{$idPop}';";}
				else
				{$Qry="INSERT INTO __t_group_akses (nmgroupakses, created_by, created_date) VALUES ('{$nmgroupakses}', '{$pcreated_by}', '{$pcreated_date}');";}
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from __t_group_akses where idgroupakses='".$idPop."'");
			$Readonly="readonly";
		break;
		case "Hapus":
			_mysql_query("delete from __t_group_akses where idgroupakses='".$idPop."'");
			$Func->kosongkanData("__t_group_akses");
			echo "<script>Fm.Action.value='BknSimpan';Fm.Mode.value='';Fm.submit();</script>";	
		break;
		default:
			if(empty($Aksi)){$Func->kosongkanData("__t_group_akses");}
	}
	...
	$Main->Isi="{$Pesan}
		<form name=Fm2 id=Fm2 method=post action='{$Url->BaseMain}/{$Pg}/{$Pr}/{$Mode}' enctype='multipart/form-data'>
			...
			<tr>
				<td style='border:0px;color:red;'>Hak&nbsp;Akses</td>
				<td>".$Func->txtField('nmgroupakses',$nmgroupakses,'','100','text',"style='width:80%'")."</td>
			</tr>
			<tr>
