<?php
	...
	$wh=!empty($ckata)?"where a.username like '%$ckata%'":""; 
	$Qry="select * from __t_users as a inner join __t_group_akses as b on a.idgroupakses=b.idgroupakses where username!='jabarsoft' $wh order by a.idgroupakses"; 
		...
		$ListMode.=" 
			<tr $wr  onmouseover=\"TG(this,'#FFCC33')\" onmouseout=\"TG(this,'$wh')\"> 
				<td align=center>$i.</td> 
 				...
 				<td>{$Isi['nomor_wa']}</td>
 				<td>{$Isi['blokir']}</td>
				<td align='center' onclick=\"javascript:showPopUp('800','200');ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/hakakses/Lihat/{$Isi['idgroupakses']}')\"><a href='#' class='btn btn-danger btn-circle' style='color:white;font-size:7pt;font-weight:none;'>".$Isi['nmgroupakses']."</a></td>
 		"; 
		if($Excel!='yes'){ 	
			$onclick_edit="onclick=\"javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Lihat/{$Isi['username']}')\"";
			$onclick_hapus="onclick=\"Cek=confirm('Apakah yakin akan menghapus data?');if(!Cek){return false;}else{javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Hapus/{$Isi['username']}')}\"";
			$ListMode.=" 
				<td>
	...
	$List="<table class='table table-nowrap align-middle' id='orderTable'>
			<thead class='text-muted table-light'>
				<tr class='text-uppercase'>
					...
					<th class='sort'>No. HP/WA</th>
					<th class='sort'>Blokir</th>
					<th class='sort'>Hak Akses</th>
	"; 
