<?php
	...
	$wh=!empty($ckata)?" and nmkatdokumen like '%$ckata%'":""; 
	$Qry="SELECT * from ref_katdokumen where idkatdokumen > 0 $wh ORDER BY nmkatdokumen "; 
		...
		$ListMode.=" 
			<tr $wr  onmouseover=\"TG(this,'#FFCC33')\" onmouseout=\"TG(this,'$wh')\"> 
				<td align=center>$i.</td> 
 				<td>{$Isi['kdkatdokumen']}</td>
 				<td>{$Isi['nmkatdokumen']}</td>
 		"; 
		if($Excel!='yes'){ 	
			$onclick_edit="onclick=\"javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Lihat/{$Isi['idkatdokumen']}')\"";
			$onclick_hapus="onclick=\"Cek=confirm('Apakah yakin akan menghapus data?');if(!Cek){return false;}else{javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Hapus/{$Isi['idkatdokumen']}')}\"";
			$ListMode.=" 
				<td>
	...
	$List="<table class='table table-nowrap align-middle' id='orderTable'>
			<thead class='text-muted table-light'>
				<tr class='text-uppercase'>
					...
					<th class='sort'>Kode Kategori Dokumen</th>
					<th class='sort'>Nama Kategori Dokumen</th>
	"; 
