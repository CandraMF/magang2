<?php
	...
	$wh=!empty($ckata)?" and a.nmdokumen like '%$ckata%'":""; 
	$Qry="SELECT * from ref_dokumen as a inner join ref_katdokumen as b on a.kdkatdokumen=b.kdkatdokumen where a.iddokumen > 0 $wh ORDER BY b.kdkatdokumen, a.kddokumen "; 
	...
	while($Isi=_mysql_fetch_array($Qry)){ 
		...
		$ListMode.=" 
			<tr $wr  onmouseover=\"TG(this,'#FFCC33')\" onmouseout=\"TG(this,'$wh')\"> 
				...
				<td>{$Isi['kdkatdokumen']} {$Isi['nmkatdokumen']}</td>
 				<td>{$Isi['kddokumen']} {$Isi['nmdokumen']}</td>
 				
 		"; 
		if($Excel!='yes'){ 	
			$onclick_edit="onclick=\"javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Lihat/{$Isi['iddokumen']}')\"";
			$onclick_hapus="onclick=\"Cek=confirm('Apakah yakin akan menghapus data?');if(!Cek){return false;}else{javascript:showPopUp(".$PopUpWidth.");ambilData('#faceboxisi', '{$Url->BaseMain}/{$Pg}/{$Pr}/detail/Hapus/{$Isi['iddokumen']}')}\"";
			$ListMode.=" 
				<td>
	...
	$List="<table class='table table-nowrap align-middle' id='orderTable'>
			<thead class='text-muted table-light'>
				<tr class='text-uppercase'>
					...
					<th class='sort'>Kategori Dokumen</th>
					<th class='sort'>Nama Dokumen</th>
