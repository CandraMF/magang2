		...
		if($method=='POST'){
			if(!empty($usrnm)){
				...
				if(!empty($idgroupakses)){
					...
					$Qry="select a.stsdok, a.idprdh, a.kdregister,a.nmdokumen,a.kddokumen, a.nmjudul, a.kdmap, b.nourut ,a.tglpengajuan, c.tglvalid, a.created_date, a.uraian from t_prohuk as a left join t_map_dokumen as b on a.kdmap=b.kdmap and a.kddokumen=b.kddokumen left join t_prohuk_valid as c on a.kdregister=c.kdregister and a.idbidang=c.idbidang where a.kdmap not in (SELECT kdmap FROM t_map_master) and a.stsdok is null and b.idgroupakses='{$idgroupakses}' {$wh} order by a.created_date desc"; 
					...
					if($jmlhalaman >= $halaman){
						...
						while($Isi=_mysql_fetch_array($Qry)){ 
							...
							$result['data'][$i-1]['tglpengajuan']=$Func->TglAll($Isi['tglpengajuan']);
