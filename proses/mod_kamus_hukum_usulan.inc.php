		...
		if($method=='POST'){
			if(!empty($usrnm)){
				...
				if(!empty($idgroupakses)){
					...
					$wh=!empty($ckata)?" and (lower(a.nmkamus) like '%".strtolower($ckata)."%' or lower(a.uraian) like '%".strtolower($ckata)."%')":"";
					$Qry="SELECT a.kdregister,a.nmkamus,  a.uraian, a.stskamus, a.created_date from t_kamus_hukum as a left join ref_bidang as b on a.idbidang=b.idbidang where a.idkamus > 0  and a.stskamus='Pengajuan' and a.idbidang ='{$idbidang}' $wh ORDER BY a.nmkamus"; 
					...
					if($jmlhalaman >= $halaman){
						...
						while($Isi=_mysql_fetch_array($Qry)){ 
							...
							$result['data'][$i-1]['stskamus']=$Isi['stskamus'];
