<?php
	...
	switch($Action)
	{
		case "Simpan";
			if(!empty($name)){
				$QTmp="select count(*) as jml from magang.region_tr where region_id='{$idSub}'";
				$Qry = _mysql_query( $QTmp );	
				$Hasil=_mysql_fetch_array($Qry);	
				$jCount=!empty($Hasil['jml'])?$Hasil['jml']:0;			
				if ($jCount > 0)
				{	
					$Qry="UPDATE magang.region_tr SET name='{$name}' WHERE  region_id='{$idSub}';";}
				else
				{$Qry="INSERT INTO magang.region_tr (name) VALUES ('{$name}');";}
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from magang.region_tr where region_id='".$idSub."'");
			$plogin=$login;
			$ParamAksi="<script>setTimeout(function(){  $('#kt_modal_1').modal('show'); }, 500);</script>";
			$Readonly="readonly";
		break;
		case "Hapus":
			_mysql_query("delete from magang.region_tr where region_id='".$idSub."'");
			$Func->kosongkanData("magang.region_tr");
		...
		break;
		default:
			if(empty($Action)){$Func->kosongkanData("magang.region_tr");}
	}
	...
	$Qry="SELECT a.region_id, a.name FROM magang.region_tr as a order by a.region_id"; 
	...
	while($Isi=_mysql_fetch_array($Qry)){ 
		...
		$onclick_edit="onclick=\"Fm.idSub.value='{$Isi['region_id']}';Fm.Action.value='Lihat';Fm.submit();\" ";
		$onclick_hapus="onclick=\"Cek=confirm('Apakah yakin akan menghapus data?');if(!Cek){return false;}else{Fm.idSub.value='{$Isi['region_id']}';Fm.Action.value='Hapus';Fm.submit();}\"";
		$td=" 
			<td>
		...
		<div class='py-10'>
			...
			<thead>
				<tr class='fw-bold fs-6 text-gray-800'>
					...
					<th class='min-w-500px'>Daerah</th>
