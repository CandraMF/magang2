<?php
	...
	switch($Aksi)
	{
		case "Simpan";
			if(!empty($company)&&!empty($position)){
				$QTmp="select count(*) as jml from magang.person_work_tx where id='{$idPopSub}'";
				...
				if ($jCount > 0)
				{	
					$Qry="UPDATE magang.person_work_tx SET person_id='{$person_id}', company='{$company}', position='{$position}', description='{$description}', start_period='{$start_period}', end_period='{$end_period}', update_date='{$pcreated_date}' WHERE  id='{$idPopSub}';";
				}else{
					$Qry="INSERT INTO magang.person_work_tx (person_id, company, position, description, start_period, end_period, create_date) VALUES ('{$person_id}', '{$company}', '{$position}', '{$description}', '{$start_period}', '{$end_period}', '{$pcreated_date}');";
				}
		...
		break;
		case "Lihat":
			$Func->ambilData("select * from magang.person_work_tx where id='".$idPopSub."'");
			$plogin=$login;
			$ParamAksi="<script>setTimeout(function(){  $('#kt_modal_1').modal('show'); }, 500);</script>";
			$Readonly="readonly";
		break;
		case "Hapus":
			_mysql_query("delete from magang.person_work_tx where id='".$idPopSub."'");
			$Func->kosongkanData("magang.person_work_tx");
		...
		break;
		default:
			if(empty($Action)){$Func->kosongkanData("magang.person_work_tx");}
	}
	...
	$Func->ambilData("select person_id from magang.person_tm where email='{$sUserId}'");
	$Qry="select id, company, position, description from magang.person_work_tx where person_id='{$person_id}'"; 
	...
	include "{$Dir->ModAdmin}{$Pr}/pekerjaan_detail.inc.php";
	$Main->Isi = "
			...
			<div class='card-header card-header-stretch'>
				...
				<div class='card-toolbar'>
					<ul class='nav nav-tabs nav-line-tabs nav-stretch fs-6 border-0'>
						...
						<li class='nav-item' >
							<a class='nav-link active' href='{$Url->BaseMain}/{$Pg}/{$Pr}/pekerjaan/baru'>Pekerjaan</a>
						</li>
						<li class='nav-item' >
